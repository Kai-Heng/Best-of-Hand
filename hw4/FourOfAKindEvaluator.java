		...
		if(allCards.length >= cardsRequired()) {
			for (int i = 0; i<allCards.length-3; i++) {
				if(allCards[i].getRank() == allCards[i+1].getRank() && allCards[i+1].getRank() == allCards[i+2].getRank() 
						&& allCards[i+2].getRank() == allCards[i+3].getRank()) {
					return true;
				}
			}
		}
